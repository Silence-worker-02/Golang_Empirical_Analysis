...
def download_addition_repo():
    ...
    db = connect_mongodb()
    table = db['snyk_vulnerabilities']
    origin = './new_vul_repos/github.com/'
    for i in table.find():
...
def generate_package_json():
    ...
    table = db['snyk_vulnerabilities']
    module_info = dict()
    for i in os.listdir('./new_vul_repos/github.com'):
        for j in os.listdir('./new_vul_repos/github.com/' + i):
            path = './new_vul_repos/github.com/' + i + '/' + j
            repo = 'github.com/' + i + '/' + j
...
def map_repo():
    ...
    for i in table.find():
        if i['package_path'] not in result.keys():
            ...
            for path in paths:
                if os.path.exists('./new_vul_repos/' + path):
...
def generate_mongo_repo_commits():
    ...
    for repo in repo_info_table.find():
        ...
        process = subprocess.Popen(command, shell=True, cwd='./new_vul_repos/' + repo['repo'],
...
def update_vul_tags():
    ...
    for repo in repo_info_table.find(no_cursor_timeout=True):
        ...
        cmd = 'git tag'
        p = subprocess.check_output(cmd, shell=True, cwd='./new_vul_repos/' + repo['repo'])
        tags = {}
        for i in p.splitlines():
            try:
                tag = str(i)[1:].replace('\'', '')
                cmd = 'git show -s --pretty=format:%H,%cd ' + str(i)[1:].replace('\'', '')
                q = subprocess.check_output(cmd, shell=True, cwd='./new_vul_repos/' + repo['repo'])
...
if __name__ == '__main__':
    crawl_snyk_vuls_by_pages()
    # generate_package_json()
    # map_repo function need change by the vul repos that downloaded
    # map_repo()
    ...
    # search_mongo_issues()
    # update_vul_tags()
    # update_final_pulls_issues()
    # search_all_fixing_commits()
    # generate_google_dependency_mongo()
